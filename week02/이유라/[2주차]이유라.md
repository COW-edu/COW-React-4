...
>    하지만 단점도 존재하는데,
> 5. 컴포넌트 기반의 아키텍처
>    컴포넌트 단위로 개발하기 때문에 많은 코드를 재사용할 수 있다. 이러한 재사용 가능한 컴포넌트는 생산성과 유지 보수를 용이하게 하여 React 의 큰 이점으로 작용한다. <br> > <br> 하지만 **React의 단점**도 존재하는데, 대표적인 단점은 큰 단점 중 하나는 SEO(검색엔진 최적화)에 불리하다는 것이다. React는 단일 페이지 애플리케이션(SPA) 방식으로 작동하기 때문에, 웹 페이지가 처음 로드될 때 클라이언트 측에서 렌더링되면서 SEO 최적화가 어려울 수 있다. 하지만 이는 SSR(Server side rendering)과 meta-tag 사용, pre-rendering 도입 등의 방식으로 극복이 가능하기 때문에 위의 장점들로 충분히 단점을 상쇄할 수 있다고 생각한다.
>
> #
